  ...
  async (user: IUser, thunkAPI) => {
    try {
      const res = await axios.post(
        'http://3.99.223.165:8000/deck/initialize',
        user
      );
  ...
  async (deckId: number | undefined, thunkAPI) => {
    try {
      const res = await axios.get(
        `http://3.99.223.165:8000/deck/getDeck/${deckId}`
      );
...
export const getDeckByUID = createAsyncThunk(
  ...
  async (userId: number | undefined, thunkAPI) => {
    try {
      const res = await axios.get(
        `http://3.99.223.165:8000/deck/getDeckByUID/${userId}`
      );
...
export const getDealPlayer = createAsyncThunk(
  ...
  async (deckId: number | undefined, thunkAPI) => {
    try {
      const res = await axios.get(`http://3.99.223.165:8000/deck/deal/${deckId}`);
      return res.data;
    } catch (e) {
      console.log(e);
    }
